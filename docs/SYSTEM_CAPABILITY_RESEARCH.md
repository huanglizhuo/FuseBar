...
> 当前实现摘要（2026-09-17）：最近使用应用架、可配置快捷键、编码布局与项目入口、输入源读取/切换及主题图标已加入开发源码；底部为设置图标 / 居中三快捷按钮 / Quit，帮助在右上角。详情采用面板内二级导航，侧边子菜单待实现。下文保留阶段性决策/审计历史；最新界面以 [README](../README.md)、[编码工作台定义](CODING_WORKSPACE_PLAN.md) 和 [验证记录](VALIDATION.md) 为准。
