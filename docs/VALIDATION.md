...
### 2026-09-25 — 全面清理重构：死代码删除与重复代码合并（UI 像素级不变）

- 死代码删除（均经引用核查为零调用方）：`PopoverView.performSearchEntry` 中 5 个被上方 `statusPages` 提前返回屏蔽的 `page = .battery/.wifi/...` 分支，及 `mainContent` 中电池/声音/Wi-Fi/蓝牙/输入源 5 个不可达整页分支（真机、渲染脚本、画廊三条路径均走子菜单；`Page` 枚举保留，渲染脚本仍引用）；`QuickAction.help`；`StatusStore.updatedAt`（只写不读）；`KeyboardSource.icon`（只写不读，图标缓存简化为仅存 template）；`searchResultButton` 恒为 false 的 `compact` 参数；`ApplicationShelf.scanning` 由 @Published 降级普通变量；五语各删 3 个孤儿键（"搜索常用和运行中的应用"、"打开系统应用启动器（Apps 或 Launchpad）"、"打开 Mission Control，查看所有窗口"）。
- 重复代码合并（共享组件，行为不变）：`SystemMenuComponents` 新增 `MenuListRow`/`MenuRefreshButton`/`menuListHeight`，三面板（Wi-Fi/蓝牙/声音）设备行、刷新按钮、列表高度上限统一；声音控件抽出 `VolumeEditModel`（含输出设备 UID 防串写守卫，首页行同步获得该防护）+ `MuteButton`（图标尺寸参数化保持两处像素不变）+ `MutedNotice`；`menuRowFocus` 修饰符收编 8 处 →/Return/↓/↑ 键盘导航四件套；`ScopedBookmark` 统一 4 文件安全作用域书签创建/解析（挂载选项差异参数保留）；`volumePercent` 统一音量百分比取整（修正首页滑杆辅助功能值未四舍五入的不一致）；`WiFiStatus.bars(forRSSI:)` 消除 `NearbyNetwork.signalFraction` 对构造状态对象的反向依赖；`nameIsBefore` 比较器 ×4；`ApplicationShelfModel.gridTiles` 提取首页网格 12 格截断逻辑（注意：既有测试专用的 `compact` 是"仅运行中"语义，与首页网格语义不同，未混用）；`LatestVolumeWriter` 删除生产未用的默认 queue 参数；`StatusStore` 两处 `AudioOutput.choices` 调用合并；默认设备名 `L("声音输出")` 提为 `defaultAudioDeviceName`。
- 刻意不动：音频双读路径（SystemReader ↔ StatusStore soundRevision）与 6 处异步 revision 样板（语义各有差异，统一抽象风险大于收益）；observer teardown 三处（Notification Center 类型不同：default/NSWorkspace/Distributed）；`ApplicationShelfModel.stable`/`StatusSnapshot.headline` 等测试锁定的行为契约；`AudioChangeMonitor.registeredPropertyCount` 测试钩子；OrbView/SideSubmenu/AnchorPointerView 展示逻辑。
